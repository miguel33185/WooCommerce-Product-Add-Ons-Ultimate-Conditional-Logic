	...
	<script>
		jQuery(document).ready(function($) {
								...
								foreach( $conditions as $condition ) {
									...
									//if( $condition['field'] == 'cost' ) {
										echo "$('body').on('input change','.qty',function(){\n
											...
											console.log( 'cost = ' + $('#pewc_total_calc_price').val() );\n
										});\n";
									//} else if( $condition['field'] == 'quantity' ) {
										echo "$('body').on('input change','.qty',function(){\n
									...
									//} else {
										if( strpos($condition['field'],'pa_')===0 || $condition['field'] == 'cost' || $condition['field'] == 'quantity' ) {
											echo "$( 'body' ).on( 'change', '#{$condition['field']}', function( e ) {\n
												pewc_check_rules_for_{$id}( $('option:selected',this).text() ) ;\n
												$('.qty').trigger('change');
											});\n";
										} else {
			...
			$('body').on('woocommerce_variation_has_changed', function(){
				// do your magic here...
				console.log("after changed--"+$('#pewc_total_calc_price').val());
			});
		});
	</script>
<?php
...
echo "</pre>";*/
			foreach( $conditions as $condition ) {
				...
				if( strpos($condition['field'],'pa_')===0) $field = "attribute_".$field;
				...
				$condition_field_id = explode( '_', $id );
				...
				if( is_array( $posted_field ) ) {
					foreach( $posted_field as $pf_key=>$pf_value ) {
						...
						//if( strpos($pf_key,'attribute_pa_')===0)
						//	$posted_field[$pf_key] = attribute_slug_to_title( $pf_key, $pf_value );
						if( strpos($pf_key,'pa_')===0){
							$term = get_term_by('slug', $pf_value, $pf_key); 
							$posted_field[$pf_key] = $term->name;
						}
					}
				} else {
					$posted_field = pewc_keyify_field( $posted_field );
					if( strpos($condition['field'],'pa_')===0){
						$term = get_term_by('slug', $posted_field, str_replace("attribute_",'',$field) ); 
						$posted_field = $term->name;
					}
				}
//echo "<pre>".$field . $rule . $value;
/*
echo "<br>posted_field: "; 			print_r($posted_field);
echo "<br>conditions: "; 			print_r($conditions);
echo "<br>condition_field_id: "; 	print_r($condition_field_id);
echo "<br>field_type: ";         	print_r($field_type);
echo "<br>items: ";   		      	print_r($items);
echo "<br>posted: ";   		      	print_r($posted);
echo "</pre>";	*/
...
//echo "<pre>match_all: is-> rules_obtain: ";echo $rules_obtain; echo "$value</pre>";
...
//echo "<pre>match_all: is_not-> rules_obtain: ";echo $rules_obtain; echo "</pre>";
...
//echo "<pre>match_all: contains-> rules_obtain: ";echo $rules_obtain; echo "</pre>";
...
//echo "<pre>match_all: cost-equal-> rules_obtain: ";echo $rules_obtain; echo "</pre>";
					...
					if( $field_type == 'upload' ) {
						...
						if( $posted_field && floatval($posted_field) > floatval($value) ) {
...
//echo "<pre>match_all: cost-greater-> rules_obtain: ";echo $rules_obtain; echo "</pre>";
...
//echo "<pre>match_all: cost-less-> rules_obtain: ";echo $rules_obtain; echo "</pre>";
...
//echo "<pre>match_all: greater-than-equals-> rules_obtain: ";echo $rules_obtain; echo "</pre>";
...
//echo "<pre>match_all: less-than-equals-> rules_obtain: ";echo $rules_obtain; echo "</pre>";
			}
			...
			foreach( $conditions as $condition ) {
				...
				if( $rule == 'is' ) {
					...
					if( $field_type == 'upload' ) {
						...
						if( floatval($posted_field) > floatval($value) ) {
